...
async function login(userCredentials) {
  ...
  const response = await httpClient
    .post("https://localhost:8080/auth/login", userCredentials)
  ...
  if (status != "401" && status != "404" && response) {
    ...
    const decoded_refresh_token = TokenService.decodeToken(
...
async function checkTokenValidity(jwt) {
  const response = await httpClient.get(
    "https://localhost:8080/auth/magic/" + jwt
  );
...
async function loginViaLink(jwt) {
  ...
  const response = await httpClient(
    "https://localhost:8080/auth/linkLogin/" + jwt
  ).catch(function (error) {
    if (error.response) {
