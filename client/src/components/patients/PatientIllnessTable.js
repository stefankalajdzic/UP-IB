...
export const PatientIllnessTable = () => {
  ...
  async function fetchData() {
    try {
      const response = await PatientBookService.getByPatient(
  ...
  function handleIllnessChange() {
    ...
    const status = PatientBookService.updateIllnesses(
      requests.id,
      illnessChanged
    )
  ...
  function handleDrugChange() {
    ...
    PatientBookService.updateDrugs(requests.id, drugChange)
    ...
    <div className="flex-container" style={{ margin: "2% 10% 2% 10%" }}>
      ...
      >
        ...
        <DialogContent>
          <TextField
            value={newIllness}
            onChange={(event) => changeInputHandlerIllness(event)}
            fullWidth
            autoFocus
            margin="dense"
            id="illnessTf"
      ...
      >
        ...
        <DialogContent>
          <TextField
            value={newDrug}
            onChange={(event) => changeInputHandlerDrug(event)}
            fullWidth
            autoFocus
            margin="dense"
            id="drugTf"
